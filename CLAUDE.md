...
**`engine/usb/UsbPassthroughManager.kt`** — runtime USB device passthrough into the QEMU guest. An unprivileged Android app cannot open the `/dev/bus/usb` device nodes, so it takes the already-open fd from `UsbManager` / `UsbDeviceConnection.getFileDescriptor()` and streams it to QEMU over `qmp.sock` as SCM_RIGHTS ancillary data (`add-fd`), then hot-plugs it with `device_add usb-host,hostdevice=/dev/fdset/N`. A code-registered `BroadcastReceiver` (no manifest `device_filter.xml`) is live only while the VM is `Running` — `PodroidService` drives `start()`/`stop()` off the VM state. Gated by the `usb_passthrough_enabled` setting, which also makes `buildCommand()` emit `-device qemu-xhci` at boot. Needs a libusb-enabled QEMU build — the `Dockerfile` cross-compiles libusb and configures QEMU `--enable-libusb`.
